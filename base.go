...
/*
Configuration for using geojsonvt!
*/
type Config struct {
	MaxZoom        int
	MinZoom        int
...
// geometry struct for geojsonvt feature
...
// feature struct for geojsonvt feature
type Feature struct {
	ID       interface{}
	Type     string
	Geometry Geometry
...
func (tile Tile) SplitTileChildren() map[m.TileID]Tile {
	...
	tl_tile, bl_tile, tr_tile, br_tile := m.TileID{tileid.X * 2, tileid.Y * 2, tileid.Z + 1}, m.TileID{tileid.X * 2, tileid.Y * 2, tileid.Z + 1}, m.TileID{tileid.X * 2, tileid.Y * 2, tileid.Z + 1}, m.TileID{tileid.X * 2, tileid.Y * 2, tileid.Z + 1}
	bl_tile.Y = bl_tile.Y + 1
	tr_tile.X = tr_tile.X + 1
	br_tile.X, br_tile.Y = br_tile.X+1, br_tile.Y+1

	// sending the left right clippers into one another
...
// this function converts a raw set of geojson features into a tile
func TileFromGeoJSON(geojsonfeatures []*geojson.Feature, tileid m.TileID, options Config) Tile {
	tile := NewTile()
